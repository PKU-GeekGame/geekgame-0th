...
def copy_flag(flag_path, new_path, token):
    with open(flag_path, "r") as f:
        flag = f.read().strip()
    msg = token.encode('latin-1') + b"A+iovbW50L2vc(U"
...
if __name__ == "__main__":
    ...
    try:
        docker = Docker("decoder:1.0")
        copy_flag("flag.txt", os.path.join(docker.mounted_path, "flag"), token)

        try:
            cmd = ["/bin/sh", "-c", "cd /chall && ./decoder"]
            proc = docker.run(cmd, stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr, timeout=10)
